...
async def repair(request: RepairRequest) -> RepairResult:
    ...
    print("REPAIR STORED:")
    print("Upload ID:", request.uploadId)
    print("ZIP PATH:", result["zip_path"])
    print("ZIP EXISTS:", Path(result["zip_path"]).exists())
    print("ACTIVE REPAIRS:", ACTIVE_REPAIRS)
    
    # --------------------------------------------------
    # Post-Repair Scan & Verification
    # --------------------------------------------------
    try:
        post_opengrep_findings = run_security_scan(repaired_path)
        post_dependency_findings = scan_dependencies(repaired_path)

        post_raw_findings = post_opengrep_findings + post_dependency_findings
